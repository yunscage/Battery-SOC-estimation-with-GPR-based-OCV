...
% GAIN=1;
% errvar=abs(Z-Zhat_k);
% if errvar>0.1 && index<1000% 当误差较大时使用Measure
%     Zi_k_1=zeros(1,m);
%     for j=1:m      
%         Zi_k_1(:,j) = Measure(Xi_k(:,j),u,Para);
%     end
%     Zhat_k1=0;
%     for j=1:m  
%         Zhat_k1=w*Zi_k_1(:,j)+Zhat_k1;      %量测预测
%     end
%     Zi_k_=0.7*Zi_k_1+0.3*Zi_k_;
%     Zhat_k=0.7*Zhat_k+0.3*Zhat_k1;
% % GAIN=2;
% % else
% %     GAIN=1;
% GAIN=15*errvar;
